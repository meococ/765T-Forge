...
**Server-only tools** (no plugin switch — handled in `ForgeToolRunner`): `forge_run_script`, `forge_batch_run`, `forge_batch_status`, `forge_system_tool_profile`, `forge_audit_summarize`, `forge_sheet_inventory_import`, `forge_issue_set_diff`, and `forge_linework_transform`. The six other `forge_linework_*` tools are **dual-path**: `ForgeToolRunner` resolves `source=dumpFile` locally through `LineworkLocal` and sends `source=drawing` over the pipe to the plugin's `PluginCommandProcessor.Linework` arm, so they have plugin switch arms. The whole family is `RequiresAutoCad=false` in the registry and `ReadOnly=false` because the tools create the caller's `outputPath`/`overlayPath`/`transformPath` artifacts. Also update `docs/capability-matrix.md`, skill/cheatsheet, and CHANGELOG.

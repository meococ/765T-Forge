...
- TFM follows the documented host CLR: 2017–2018 `net46` (.NET Framework 4.6), 2019–2020 `net47`, 2021–2024 `net48`, 2025–2026 `net8.0-windows`. `System.Text.Json` 8.0.5 cannot restore for `net46` (NU1202). `net461` restores only with an “unsupported TFM” warning and is newer than CLR 4.6, so it is not used. 2017–2018 JSON goes through Newtonsoft.Json. Those binaries are **not smoke-tested**. `forge_exec_dotnet` on those years returns `autocad_version_unsupported`. Packed bundles list only years whose DLL was built.

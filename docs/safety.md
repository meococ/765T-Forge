...
- **Server-only tools** (`forge_batch_status`, `forge_system_tool_profile`, `forge_audit_summarize`, `forge_sheet_inventory_import`, `forge_issue_set_diff`, and `forge_linework_transform`) never cross the pipe. `forge_linework_transform` is pure coordinate math and only writes the caller-supplied `transformPath` artifact. The other six `forge_linework_*` tools are dual-path: `source=dumpFile` resolves on the server without AutoCAD, and `source=drawing` is dispatched over the pipe and therefore goes through the plugin's safety gate as well. Every linework tool writes only the caller-supplied `outputPath` / `overlayPath` / `transformPath` artifacts and never edits a drawing.

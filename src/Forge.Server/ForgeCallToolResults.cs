...
/// <summary>
/// MCP SDK 1.3.0 leaves <c>IsError</c> unset when a tool returns a plain object. A Forge
/// result with <c>Ok=false</c> is a business failure and must reach the agent as a tool
/// error, not as a successful call whose payload happens to say <c>ok:false</c>.
/// </summary>
public static class ForgeCallToolResults
{
    /// <summary>
    /// Tools do not advertise an output schema, so a ForgeResult arrives either as structured
    /// content or as JSON text. Business failure sets IsError and keeps the ForgeResult object

...
public sealed partial class PluginCommandProcessor
{
    ...
    private static LineworkLoadResult LoadLinework(ForgeCommand command, LineworkQueryArgs args)
    {
        ...
        try
        {
            insunits = Convert.ToInt32(
                Autodesk.AutoCAD.ApplicationServices.Application.GetSystemVariable("INSUNITS"),
                CultureInfo.InvariantCulture);
        }
        catch (System.Exception ex) when (ex is Autodesk.AutoCAD.Runtime.Exception or FormatException or InvalidCastException or OverflowException)
        {
            // INSUNITS = 0 is AutoCAD's defined "Unitless" value, so falling through to it
            // records "no unit information" rather than inventing a unit. A caller that needs
            // a real unit must set INSUNITS in the drawing.
            insunits = 0;
        }
    ...
    private static LineworkEntity MakeEntity(ObjectId id, LineworkKind kind, string layer, Vec2[] pts, bool closed, double radius = 0)
    {
        ...
        {
            Id = 0,
            Handle = id.Handle.Value.ToString("X", CultureInfo.InvariantCulture),
            Kind = kind,
            Layer = layer,

...
public static class LineworkSvg
{
        ...
        string title)
    {
        ...
        sb.Append('\n');
        AppendInvariant(sb, $"<title>{Esc(title)}</title>\n");
        ...
        var ly = minY + h * 0.02;
        var fs = h * 0.016;
        AppendInvariant(sb, $"""<g font-family="monospace" font-size="{fs:0.###}">""");
        sb.Append('\n');
        AppendInvariant(sb,
    ...
    /// <summary>Invariant-culture append (netstandard2.0 has no StringBuilder.Append(IFormatProvider, ...)).</summary>

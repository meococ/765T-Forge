...
/// the plugin calls <see cref="ExecuteOnEntities"/> after extracting from the live drawing.
/// One code path = identical results either way.
/// </summary>
public static class LineworkLocal
{
    ...
    /// <summary>Full local resolution for the MCP server (dumpFile source only — drawing mode goes to the plugin).</summary>
    public static ForgeResult Execute(ForgeCommand command)
    {
        var baseArgs = ForgeJson.FromElement<LineworkQueryArgs>(command.Args) ?? new LineworkQueryArgs();
        ...
        return command.Tool.ToLowerInvariant() switch
        {
            ...
            "forge_linework_trace" => ExecuteTrace(command,
                ForgeJson.FromElement<LineworkTraceArgs>(command.Args) ?? new LineworkTraceArgs(), entities, source),
            "forge_linework_topology" => ExecuteTopology(command,
                ForgeJson.FromElement<LineworkTopologyArgs>(command.Args) ?? new LineworkTopologyArgs(), entities, source),
            "forge_linework_coverage" => ExecuteCoverage(command,
                ForgeJson.FromElement<LineworkCoverageArgs>(command.Args) ?? new LineworkCoverageArgs(), entities, source),
            "forge_linework_segments" => ExecuteSegments(command,
                ForgeJson.FromElement<LineworkSegmentsArgs>(command.Args) ?? new LineworkSegmentsArgs(), entities, source),
            "forge_linework_compare" => ExecuteCompare(command,
                ForgeJson.FromElement<LineworkCompareArgs>(command.Args) ?? new LineworkCompareArgs(), entities, source),
        ...
        SourceInfo source)
    {
        return command.Tool.ToLowerInvariant() switch
        {
            "forge_linework_dump" => ExecuteDump(command, baseArgs, entities, source),
            "forge_linework_trace" => ExecuteTrace(command,
                baseArgs as LineworkTraceArgs ?? ForgeJson.FromElement<LineworkTraceArgs>(command.Args) ?? new LineworkTraceArgs(), entities, source),
            "forge_linework_topology" => ExecuteTopology(command,
                baseArgs as LineworkTopologyArgs ?? ForgeJson.FromElement<LineworkTopologyArgs>(command.Args) ?? new LineworkTopologyArgs(), entities, source),
            "forge_linework_coverage" => ExecuteCoverage(command,
                baseArgs as LineworkCoverageArgs ?? ForgeJson.FromElement<LineworkCoverageArgs>(command.Args) ?? new LineworkCoverageArgs(), entities, source),
            "forge_linework_segments" => ExecuteSegments(command,
                baseArgs as LineworkSegmentsArgs ?? ForgeJson.FromElement<LineworkSegmentsArgs>(command.Args) ?? new LineworkSegmentsArgs(), entities, source),
            "forge_linework_compare" => ExecuteCompare(command,
                baseArgs as LineworkCompareArgs ?? ForgeJson.FromElement<LineworkCompareArgs>(command.Args) ?? new LineworkCompareArgs(), entities, source),
            _ => ForgeResult.Failure(command.Id, "unknown_tool", $"Unknown linework tool: {command.Tool}")
        };
    }

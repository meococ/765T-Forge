...
public sealed class LayerMatcher
{
    ...
    public LayerMatcher(string? filter, string? mode = null)
    {
        ...
        _patterns = SplitTrimmed(text, ',')
            .Select(p => (p, p.IndexOf('*') >= 0 || p.IndexOf('?') >= 0 ? WildcardToRegex(p) : null))
            .ToArray();
    ...
    private bool MatchesPlain(string layer, string tail, string pattern)
    {
        return _mode switch
        {
            ...
            LayerMatchMode.Substring => layer.IndexOf(pattern, StringComparison.OrdinalIgnoreCase) >= 0,
            ...
            _ => // Auto: exact, tail-exact, or substring anywhere (prefix-aware).
                string.Equals(layer, pattern, StringComparison.OrdinalIgnoreCase) ||
                string.Equals(tail, pattern, StringComparison.OrdinalIgnoreCase) ||
                layer.IndexOf(pattern, StringComparison.OrdinalIgnoreCase) >= 0
        };
    }
    ...
    public static string Tail(string layer)
    {
        ...
        return idx >= 0 ? layer.Substring(idx + 1) : layer;
    }

    /// <summary>netstandard2.0 has no StringSplitOptions.TrimEntries; explicit equivalent.</summary>
    internal static string[] SplitTrimmed(string text, char separator)
    {

...
- `forge_linework_*` tool family (`dump`, `trace`, `topology`, `coverage`, `segments`, `compare`, `transform`) — CAD linework extraction and CAD↔model QA with xref-safe layer filtering, a flat segment feed, per-item `matched`/`partial`/`missing_in_revit`/`extra_off_cad` marking, an optional SVG overlay, and anchor-pair similarity/affine calibration. `source=dumpFile` reads a `pl_dump.txt` dump and resolves server-side without AutoCAD; `source=drawing` extracts from the live drawing through the plugin's `PluginCommandProcessor.Linework` arm; `forge_linework_transform` is server-only. Unit-tested only — not yet exercised against a real drawing or a real Revit/pipe export
